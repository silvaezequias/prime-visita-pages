...
export const HomeView: React.FC<HomeViewProps> = ({ setTab }) => {
  ...
  const [simulatorVisits, setSimulatorVisits] = useState<SimulatorVisit[]>([
    { id: '1', doctor: 'Dra. Helena Souza', specialty: 'Cardiologia', time: '09:00', status: 'concluida', priority: 'A' },
    { id: '2', doctor: 'Dr. Arthur Lima', specialty: 'Pediatria', time: '11:30', status: 'concluida', priority: 'B' },
    { id: '3', doctor: 'Dra. Camila Nogueira', specialty: 'Ortopedia', time: '14:15', status: 'agendada', priority: 'A' },
    { id: '4', doctor: 'Dr. Roberto Mendes', specialty: 'Dermatologia', time: '16:00', status: 'agendada', priority: 'C' },
  ]);
  ...
  const steps = [
    { title: '1. Contratação', desc: 'Sua empresa contrata o plano e o ambiente dedicado é liberado instantaneamente.' },
    { title: '2. Configuração', desc: 'O administrador importa a planilha de médicos e define os representantes.' },
    { title: '3. Planejamento', desc: 'Os representantes montam sua agenda de visitas da semana de forma inteligente.' },
    { title: '4. Execução', desc: 'Em campo (web ou celular), as visitas são concluídas e as observações relatadas.' },
    { title: '5. Decisão', desc: 'Os gestores analisam relatórios de cobertura geográfica e tomam decisões ágeis.' },
  ];
  ...
  const handleToggleStatus = (id: string) => {
    setSimulatorVisits(
      simulatorVisits.map((visit) =>
        visit.id === id
          ? { ...visit, status: visit.status === 'concluida' ? 'agendada' : 'concluida' }
          : visit
      )
    );
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-6 space-y-6 text-left">
              ...
              <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-950 tracking-tight leading-[1.1]">
                Controle inteligente de <span className="text-blue-600 relative inline-block">visitação médica</span> sem planilhas.
              </h1>
              
              <p className="text-base sm:text-lg text-slate-600 max-w-xl leading-relaxed">
                O <strong>Prime Visita</strong> é um ecossistema inteligente em desenvolvimento projetado para organizar médicos, otimizar roteiros e dar inteligência a representantes de campo. Cadastre-se na Beta Privada para testar hoje mesmo.
              </p>
              ...
              >
                ...
                <div className="bg-slate-900 px-6 py-4 flex items-center justify-between text-white">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-slate-400 ml-2">Painel do Representante (Simulador)</span>
                  </div>
                ...
                <div className="p-5 sm:p-6 space-y-6 bg-slate-50/50">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-white p-3 rounded-xl border border-slate-200 text-center">
                      <p className="text-[10px] font-mono uppercase tracking-wider text-slate-400">Médicos</p>
                    ...
                    <div className="bg-emerald-50/30 p-3 rounded-xl border border-emerald-100 text-center">
                      <p className="text-[10px] font-mono uppercase tracking-wider text-emerald-600">Concluídas</p>
                      <p className="text-xl sm:text-2xl font-bold text-emerald-700">{stats.concluded}</p>
                    </div>
                    <div className="bg-amber-50/30 p-3 rounded-xl border border-amber-100 text-center">
                      <p className="text-[10px] font-mono uppercase tracking-wider text-amber-600">Pendentes</p>
                      <p className="text-xl sm:text-2xl font-bold text-amber-700">{stats.scheduled}</p>
                    </div>
                  </div>

                  {/* Simulator Quick Action Form */}
                  <form onSubmit={handleAddVisit} className="bg-white p-4 rounded-xl border border-slate-200 space-y-3 shadow-2xs">
                  ...
                  <div className="space-y-2">
                    <p className="text-xs font-mono text-slate-400 uppercase tracking-wider flex justify-between items-center px-1">
                      ...
                      <span className="text-[10px] font-normal italic lowercase text-slate-400">Clique na caixa para alternar check-in</span>
                    </p>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded-md flex items-center justify-center border transition-all ${
                              ...
                              {visit.status === 'concluida' && <Check className="w-3.5 h-3.5 stroke-[3px]" />}
                            </div>
                            <div className="text-left">
                              <p className={`text-xs font-bold ${visit.status === 'concluida' ? 'line-through text-slate-500' : 'text-slate-800'}`}>
                                {visit.doctor}
                              </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto space-y-4">
            ...
            <p className="text-slate-600 leading-relaxed text-sm sm:text-base">
              Acompanhar o relacionamento com médicos em planilhas dispersas gera ruídos, furos de agenda e falta de indicadores comerciais de mercado. O Prime Visita centraliza tudo para que você tome decisões ágeis.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mt-12 text-center">
            <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-3xs space-y-2">
              <span className="text-4xl font-extrabold text-blue-600 font-display">85%</span>
              <p className="text-sm font-semibold text-slate-800">Menos Tempo Operacional</p>
              <p className="text-xs text-slate-500">Elimine a conferência manual de planilhas após cada visita semanal de campo.</p>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-3xs space-y-2">
              <span className="text-4xl font-extrabold text-blue-600 font-display">100%</span>
              <p className="text-sm font-semibold text-slate-800">Rastreabilidade Histórica</p>
              <p className="text-xs text-slate-500">Histórico completo de visitas, amostras de medicamentos entregues e feedbacks.</p>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-200/50 shadow-3xs space-y-2">
              <span className="text-4xl font-extrabold text-blue-600 font-display">40%</span>
              <p className="text-sm font-semibold text-slate-800">Aumento de Visitas Efetivas</p>
              <p className="text-xs text-slate-500">Agenda integrada por geolocalização e rotas mais inteligentes de viagem.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          ...
          <p className="text-slate-600 text-sm sm:text-base">
            O Prime Visita conta com tudo o que sua equipe médica de relacionamento precisa para executar e acompanhar as visitas de forma impecável.
          </p>
        </div>
      ...
      <section className="bg-blue-900/5 border-y border-blue-100 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="max-w-3xl mx-auto space-y-4 mb-12">
            ...
            <p className="text-slate-600 text-sm sm:text-base">
              Simplificamos a implantação para que você comece a monitorar suas visitas em tempo recorde.
            </p>
          </div>
            ...
            <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 flex flex-col justify-between text-left shadow-xs">
              <div className="space-y-4">
                ...
                <p className="text-slate-600 text-sm leading-relaxed">
                  {steps[activeStep].desc} No Prime Visita, desenhamos esse fluxo especificamente para garantir que a transição entre as planilhas analógicas antigas e o nosso portal unificado aconteça sem perdas e sem atrito para sua equipe médica de relacionamento em campo.
                </p>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex items-center gap-4 text-xs text-slate-500">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Implementação no mesmo dia</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="max-w-3xl mx-auto px-4 sm:px-6">
        <div className="text-center space-y-4 mb-12">
          ...
          <p className="text-slate-500 text-sm">
            Tem alguma pergunta sobre a plataforma do Prime Visita? Veja as respostas abaixo.
          </p>
        </div>
        ...
        <div className="bg-slate-950 text-white rounded-3xl p-8 sm:p-12 md:p-16 relative overflow-hidden text-center space-y-6 shadow-2xl">
          ...
          <div className="relative z-10 max-w-2xl mx-auto space-y-6">
            ...
            <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
              Assuma o controle da sua operação, acabe com as planilhas dispersas e esteja pronto para o lançamento do Prime Visita. Garanta sua vaga no programa Beta Privado.
            </p>
            ...
            <p className="text-slate-500 text-[11px] font-mono">
              Ambiente de testes gratuito • Sem contratos de fidelidade • Suporte técnico dedicado
            </p>
          </div>
        </div>
      </section>
